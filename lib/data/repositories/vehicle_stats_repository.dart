...
class VehicleStatsRepository {
  ...
  Future<void> incrementScanCount({
    ...
    try {
      ...
      final query = _db.select(_db.vehicleStats)
        ..where((tbl) =>
      ...
      if (existing == null) {
        ...
        await _db.into(_db.vehicleStats).insert(
              VehicleStatsCompanion.insert(
                year: year,
                make: make,
                model: model,
        ...
        await (_db.update(_db.vehicleStats)
              ..where((tbl) => tbl.id.equals(existing.id)))
            .write(
          VehicleStatsCompanion(
  ...
  Future<List<VehicleStat>> getTopScannedModels({int limit = 5}) async {
    final query = _db.select(_db.vehicleStats)
      ..orderBy([
        (tbl) =>
  ...
  Future<void> updateGlobalScore(int id, int cloudTrendScore) async {
    ...
    final existing = await (_db.select(_db.vehicleStats)
    ...
    await (_db.update(_db.vehicleStats)..where((tbl) => tbl.id.equals(id)))
        .write(
      VehicleStatsCompanion(priorityScore: Value(newPriority)),
    );
  }
}
